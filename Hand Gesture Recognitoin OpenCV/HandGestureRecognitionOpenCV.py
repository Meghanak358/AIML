import cv2
import numpy as np
import math

cap = cv2.VideoCapture(0)  # try 0 first
if not cap.isOpened():
    print("Error: Could not open camera")
    exit()

while True:
    ret, img = cap.read()
    if not ret or img is None:
        print("Failed to grab frame")
        continue

    cv2.rectangle(img, (50, 50), (400, 400), (0, 255, 0), 2)
    crop_img = img[50:400, 50:400]

    grey = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
    blurred = cv2.GaussianBlur(grey, (35, 35), 0)
    _, thresh1 = cv2.threshold(blurred, 127, 255,
                               cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    cv2.imshow('Thresholded', thresh1)

    contours, _ = cv2.findContours(thresh1.copy(),
                                   cv2.RETR_TREE,
                                   cv2.CHAIN_APPROX_NONE)

    if len(contours) == 0:
        continue  # skip if no contour detected

    cnt = max(contours, key=cv2.contourArea)

    x, y, w, h = cv2.boundingRect(cnt)
    cv2.rectangle(crop_img, (x, y), (x+w, y+h), (0, 0, 255), 2)

    hull = cv2.convexHull(cnt)
    drawing = np.zeros(crop_img.shape, np.uint8)
    cv2.drawContours(drawing, [cnt], 0, (0, 255, 0), 2)
    cv2.drawContours(drawing, [hull], 0, (0, 0, 255), 2)

    hull = cv2.convexHull(cnt, returnPoints=False)
    if hull is not None and len(hull) > 3:
        defects = cv2.convexityDefects(cnt, hull)
    else:
        defects = None

    count_defects = 0
    if defects is not None:
        for i in range(defects.shape[0]):
            s, e, f, d = defects[i, 0]
            start, end, far = tuple(cnt[s][0]), tuple(cnt[e][0]), tuple(cnt[f][0])

            a = math.dist(start, end)
            b = math.dist(start, far)
            c = math.dist(end, far)

            angle = math.acos((b**2 + c**2 - a**2) / (2*b*c)) * 57

            if angle <= 90:
                count_defects += 1
                cv2.circle(crop_img, far, 5, [0, 0, 255], -1)

            cv2.line(crop_img, start, end, [0, 255, 0], 2)

    if count_defects == 1:
        cv2.putText(img, "GESTURE ONE", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
    elif count_defects == 2:
        cv2.putText(img, "GESTURE TWO", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
    elif count_defects == 3:
        cv2.putText(img, "GESTURE THREE", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
    elif count_defects == 4:
        cv2.putText(img, "GESTURE FOUR", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)
    else:
        cv2.putText(img, "Hello World!!!", (50, 50),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0, 255), 2)

    cv2.imshow('Gesture', img)
    all_img = np.hstack((drawing, crop_img))
    cv2.imshow('Contours', all_img)

    if cv2.waitKey(1) & 0xFF == 27:  # ESC to quit
        break

cap.release()
cv2.destroyAllWindows()
